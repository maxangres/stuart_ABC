run.lavaan <-
  function(
    ...
    output.model = FALSE,
    objective = NULL, ignore.errors = FALSE, iteration_limit = NA
  ) { #begin function
    ...
    # if ('iteration_limit' %in% names(as.list(match.call(expand.dots = TRUE))[-1])) {
    #   #run.options$iteration_limit <- iteration_limit
    #   analysis.options$control <- list(iter.max=iteration_limit) 
    # } 
    ...
    output <- try(suppressWarnings(do.call(lavaan::lavaan,analysis.options)),silent=FALSE)
    ...
    if (inherits(output, 'try-error')) {
      ...
      op <- list()
      op$sol <- NA
      op$technicals$iterations <- output@optim$iterations
      op$technicals$converged <- output@optim$converged
      warning('The lavaan input generated an error.',call.=FALSE)
      return(output=op) #list(NA,technicals)
    }
    ...
    if (inherits(output, 'lavaan')) {
      if (!ignore.errors) {
        ...
        op$technicals$iterations <- output@optim$iterations
        op$technicals$converged <- output@optim$converged
        if (!suppressWarnings(lavaan::inspect(output, 'post.check'))) return(op) # = list(NA, technicals)
      }
      ...
      if (inherits(fit, 'try-error')) {
        ...
        op <- list()
        op$sol <- NA
        op$technicals <- technicals
        return(output) #=list(NA, technicals)
