...
#' @param limit A number adaptable to the state of the search that broadens or focuses the search space. Defaults to 100
#### Employed Para
#' @param ls.sel The selected method whether new foodsource is selected for every limit iteration, can be \code{'narrow'} for ongoing item selection from first selected food source or \code{'roulette'} or \code{'wide'} for new selected solution for every iteration
#### Onlooker Paras
#' @param selection if tournament or proportional selection is applied
#' @param ls.onlooker.sel The selected method by which onlooker bees select there food sources. Can be either \code{'roulette'} for roulette random selection or \code{'tournament'} (the default) for a semi-deterministic selection.
#### Scout Params
#' @param scout.selection what kind of selection for repair operator
#' @param cutoff percentage of solution that will be eliminated in the repair operator
#' @param size_best_fs how many of the best solutions are saved
#' @param size_rep_t size of the tournament for repair items
#' @param number_gen number of generation over which scout bees need to replace whole population without improvement (maybe second parameter needed)
#' General
...
#' @param convergence.criterion The criterion by which convergence is determined. Can be one of four criteria \code{'variance'}, \code{'median'}, \code{'geno.within'}, and \code{'geno.between'}, \code{'scout'}, \code{'none'} . See 'details'.
#' @param number_gen number of generations over which a certain amount of scout iniatiations has to exceed threshhold
#' @param per_gen number of scout initation per generation constituting the threshhold
...
abc <-
  function(
    ...
    objective=NULL, ignore.errors=FALSE, opt.lavaan.limit = TRUE,                    #fitness specs
    
    generations = 256, individuals = 32, limit = 10,                                  #algorithm specs
    ls.sel = 'wide', ls.onlooker.sel = 'narrow', selection.pressure = NULL,
    selection = "tournament", scout.selection = "tournament", cutoff = 0.5, size_best_fs = 10, size_rep_t = 2, 
    convergence.criterion = 'geno.between', number_gen = 5, tolerance = NULL,
    schedule = 'run',
    ...
    if (!(all(convergence.criterion %in% c('variance', 'median', 'geno.within', 'geno.between', 'scout', 'none')))) {
      stop('Invalid convergence criterion entered.', call. = FALSE)
    }
    
    #check abc individual parameters
    if(opt.lavaan.limit != TRUE) warning('No limit for CFA iterations in the lavaan package will be calculated. Please expect drastically increased algorithmic runtime.', immediate. = TRUE)
    if(ls.sel != "narrow" & ls.sel != "wide") stop('Employed Bee search spectrum input is not valid. Please choose either "narrow" or "wide"')
    if(ls.onlooker.sel != "narrow" & ls.onlooker.sel != "wide") stop('Employed Bee search spectrum input is not valid. Please choose either "narrow" or "wide"')
    if(selection != "proportionate" & selection != "tournament") stop('Selection criteria input is not valid. Please choose either "proportionate" or "tournament"')
    
    if(scout.selection != "proportionate" & scout.selection != "tournament" & scout.selection != "random") stop('Scout item selection criteria input is not valid. Please choose either "random", "proportionate" or "tournament"')
    if(cutoff > 1 | cutoff < 0) stop('Repair operator cutoff must be a value between 0 and 1')
    if(size_rep_t > capacity) stop('Repair operator tournament size must not exceed scale size')
    ...
    solution <- do.call('stuart.abc',args)
    ...
    output$duration <- solution$duration
